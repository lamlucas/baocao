...
function capNhatHienThiSoDuDauDoc() {
  ...
  if (!valEl) return;
  if (!isRevealed()) {
    valEl.textContent = "Đang ẩn — bấm “Hiện số dư đầu” để xem.";
    if (rawEl) {
      rawEl.hidden = true;
      rawEl.textContent = "";
    }
...
function capNhatHienThiBienDong() {
  ...
  if (!el) return;
  if (!isRevealed()) {
    el.textContent = "Đang ẩn — bấm “Hiện số dư đầu” để xem.";
    return;
  }
...
function refreshComputedFromClient() {
  ...
  for (const r of state.thuChi) {
    const has = `${r.ngay ?? ""}${r.thu ?? ""}${r.chi ?? ""}${r.ghiChu ?? ""}`.trim();
    if (!has) continue;
    e2 += parseNumClient(r.thu) - parseNumClient(r.chi);
  }
  let sumCocB = 0;
  let sumCocC = 0;
  ...
  $("#tq-b2").value = fmtMoney(sumCocB);
  $("#tq-c2").value = fmtMoney(sumCocC);
  $("#tq-d2").value = fmtMoney(sumNo);
  $("#tq-e2").value = String(e2);
  state.computed = {
    tongCoc: sumCocB,
    nhanCoc: sumCocC,
    tongCongNo: sumNo,
...
function addRow(kind) {
  if (kind === "thu_chi") {
    state.thuChi.push({ ngay: "", thu: "", chi: "", ghiChu: "" });
    renderThuChi();
  }
  if (kind === "coc") {
...
function applyPayload(data) {
  ...
  if (!state.thuChi.length) state.thuChi.push({ ngay: "", thu: "", chi: "", ghiChu: "" });
...
async function saveSheet() {
  ...
  const body = {
    ...
    thuChi: state.thuChi.filter((r) => (r.ngay || r.thu || r.chi || r.ghiChu).trim()),
  ...
  try {
    ...
    $("#tq-b2").value = fmtMoney(parseNumClient(out.tongQuan.b2));
    $("#tq-c2").value = fmtMoney(parseNumClient(out.tongQuan.c2));
    $("#tq-d2").value = fmtMoney(parseNumClient(out.tongQuan.d2));
    $("#tq-e2").value = fmtMoney(parseNumClient(out.tongQuan.e2));
    refreshComputedFromClient();
