...
function sumChiTieuEntries(rows) {
  ...
  let tongThu = 0;
  for (const r of rows) {
    tongTieu += parseNumClient(r.tongTieu);
    tongThu += parseNumClient(r.tongThu);
  }
...
function groupChiTieuByMcc(rows) {
  const map = new Map();
  for (const r of rows) {
    const key = String(r.mcc || "—").trim() || "—";
    const cur = map.get(key) ?? { mcc: key, tongTieu: 0, tongThu: 0, nguons: new Set() };
    cur.tongTieu += parseNumClient(r.tongTieu);
    cur.tongThu += parseNumClient(r.tongThu);
...
function chiTieuByMonthFromEntries(entries) {
  ...
  for (const e of entries) {
    ...
    cur.tongTieu += parseNumClient(e.tongTieu);
    cur.tongThu += parseNumClient(e.tongThu);
    map.set(thang, cur);
  }
...
function chiTieuByNguonForMonth(entries, month) {
  const map = new Map();
  for (const e of entries) {
    if (!String(e.ngay || "").startsWith(`${month}-`)) continue;
    const key = e.nguon || "—";
...
function renderChiTieuMccTable(tbodyEl, groups) {
  ...
  for (const g of groups) {
    ...
    tr.innerHTML = `
      ...
      <td class="cell-readonly cell-num">${fmtMoney(g.tongTieu)}</td>
      <td class="cell-readonly cell-num">${fmtMoney(g.tongThu)}</td>
      <td class="cell-readonly">${escapeHtml(g.nguon || "—")}</td>`;
    tbodyEl.appendChild(tr);
  }
}

function renderChiTieuLinesTable(tbodyEl, rows) {
  if (!tbodyEl) return;
  tbodyEl.innerHTML = "";
  if (!rows.length) {
    const tr = document.createElement("tr");
    tr.innerHTML = `<td colspan="5" class="muted">Không có dòng chi tiết.</td>`;
    tbodyEl.appendChild(tr);
    return;
  }
  for (const r of rows) {
    ...
    tr.innerHTML = `
      ...
      <td class="cell-readonly">${escapeHtml(r.taiKhoan ?? "")}</td>
      <td class="cell-readonly cell-num">${fmtMoney(parseNumClient(r.tongTieu))}</td>
      <td class="cell-readonly cell-num">${fmtMoney(parseNumClient(r.tongThu))}</td>
      <td class="cell-readonly">${escapeHtml(r.nguon ?? "—")}</td>`;
    tbodyEl.appendChild(tr);
  }
}
...
function renderChiTieuToday() {
  ...
  if (elTieu) elTieu.textContent = `Tổng tiêu: ${fmtMoney(totals.tongTieu)}`;
  if (elThu) elThu.textContent = `Tổng thu: ${fmtMoney(totals.tongThu)}`;
...
function renderChiTieuDrill() {
  ...
  if (!month && tbMonth) {
    tbMonth.innerHTML = "";
    const months = chiTieuByMonthFromEntries(entries).sort((a, b) =>
      String(b.thang).localeCompare(String(a.thang)),
    );
    for (const r of months) {
      const tr = document.createElement("tr");
      ...
      tr.innerHTML = `
        <td>${escapeHtml(formatMonthForDisplay(r.thang))}</td>
        <td class="cell-num">${fmtMoney(r.tongTieu)}</td>
        <td class="cell-num">${fmtMoney(r.tongThu)}</td>`;
      tbMonth.appendChild(tr);
    }
    ...
    if (tbNguon) {
      tbNguon.innerHTML = "";
      const latestMonth = months[0]?.thang;
      if (latestMonth) {
        for (const n of chiTieuByNguonForMonth(entries, latestMonth)) {
          const tr = document.createElement("tr");
          tr.innerHTML = `
            <td>${escapeHtml(n.nguon)}</td>
            <td class="cell-num">${fmtMoney(n.tongTieu)}</td>
            <td class="cell-num">${fmtMoney(n.tongThu)}</td>`;
          tbNguon.appendChild(tr);
        }
      } else {
        const tr = document.createElement("tr");
        tr.innerHTML = `<td colspan="3" class="muted">Chưa có dữ liệu theo tháng.</td>`;
        tbNguon.appendChild(tr);
      }
    }
  }
  ...
  if (month && !day && tbDays) {
    tbDays.innerHTML = "";
    for (const r of chiTieuByDayFromEntries(entries, month)) {
      const tr = document.createElement("tr");
      tr.className = "report-tc-click-row";
      tr.dataset.day = String(r.date);
      tr.innerHTML = `
  ...
  if (day) {
    ...
    renderChiTieuLinesTable($("#table-chi-tieu-detail-lines tbody"), dayRows);
  }
}
