...
/** Tab Báo cáo: chỉ dòng ngày nay & tháng nay (bán dao). */
function reportBanDaoRowsForBaoCaoPanel() {
  const today = todayIsoVietnam();
  const month = currentMonthIsoVietnam();
  const byDay = state.reportBanDao.byDay ?? [];
  const byMonth = state.reportBanDao.byMonth ?? [];
  return {
    byDay: byDay.filter((r) => String(r.date ?? "").trim().slice(0, 10) === today),
    byMonth: byMonth.filter((r) => String(r.thang ?? "").trim().slice(0, 7) === month),
  };
}
...
function renderReport() {
  ...
  const repBdPanel = reportBanDaoRowsForBaoCaoPanel();
  const tbBdRepDay = $("#table-report-bd-day tbody");
  if (tbBdRepDay) {
    tbBdRepDay.innerHTML = "";
    for (const r of repBdPanel.byDay) {
      const tr = document.createElement("tr");
      tr.innerHTML = `<td>${escapeHtml(formatDayForDisplay(r.date))}</td><td>${fmtMoney(r.tong)}</td>`;
      tbBdRepDay.appendChild(tr);
    }
  }
  const tbBdRepMonth = $("#table-report-bd-month tbody");
  if (tbBdRepMonth) {
    tbBdRepMonth.innerHTML = "";
    for (const r of repBdPanel.byMonth) {
      const tr = document.createElement("tr");
