    ...
    .map((period, idx) => {
      ...
      const rows = (period.employees ?? [])
        ...
        <tr>
          ...
          <td class="cell-readonly cell-num luong-nv-total">${fmtVnd(emp.baseSalaryVnd)} + ${fmtChiTieuUsd(emp.commissionUsd)}</td>
        </tr>`,
        )
        .join("");
      ...
      const emptyRow =
        rows ||
        `<tr><td colspan="5" class="muted">Chưa có tab nhân viên trong file chấm công.</td></tr>`;
      ...
      <section class="luong-nv-period" data-period="${idx}">
        ...
        <div class="luong-nv-commission-summary muted small">
          ...
          ${cb.thuExcluded > 0 ? ` (loại thu công nợ ROKER/GM: ${cellMoneyDisplay(cb.thuExcluded)})` : ""}
