...
function bindHhLoaiTruForm() {
  ...
  $("#btn-hh-loai-tru-save")?.addEventListener("click", async () => {
    ...
    try {
      ...
      state.hhLoaiTru = res.exclusions ?? state.hhLoaiTru;
      renderHhLoaiTruTable();
  ...
  wrap.innerHTML = periods
    .map((period, idx) => {
      ...
      const rows = (period.employees ?? [])
        .map(
          (emp) => {
            const phat = emp.tienPhatUsd ?? emp.tienPhatVnd ?? 0;
            const thuong = emp.tienThuongUsd ?? 0;
            const ung = emp.tienUngUsd ?? 0;
            const totalParts = [
              fmtVnd(emp.baseSalaryVnd),
              fmtChiTieuUsd(emp.commissionUsd),
            ];
            if (phat > 0) totalParts.push(`− ${fmtChiTieuUsd(phat)}`);
            if (thuong > 0) totalParts.push(`+ ${fmtChiTieuUsd(thuong)}`);
            if (ung > 0) totalParts.push(`− ${fmtChiTieuUsd(ung)}`);
            return `
        <tr>
          ...
          <td class="cell-readonly cell-num">${fmtVnd(emp.baseSalaryVnd)}</td>
          ...
          <td class="cell-readonly cell-num luong-nv-total">${totalParts.join(" ")}</td>
        </tr>`;
          },
        )
        .join("");
