...
async function loadChamCongNvTabs() {
  try {
    ...
    state.chamCongNvTemplate = res.templateTab ?? "SU_BEO";
...
function renderChamCongNvTable() {
  ...
  if (!list.length) {
    const tr = document.createElement("tr");
    tr.innerHTML = `<td colspan="2" class="muted">Chưa có tab nhân viên (ngoài mẫu ${escapeHtml(state.chamCongNvTemplate)}).</td>`;
    tb.appendChild(tr);
    return;
  }
  list.forEach((tabName) => {
    ...
    tr.innerHTML = `
      ...
      <td class="cell-readonly"><button type="button" class="btn ghost btn-sm" data-cc-del="${escapeAttr(tabName)}">Xóa</button></td>`;
    tb.appendChild(tr);
  });
...
function bindChamCongNvPanel() {
  ...
  $("#btn-cham-cong-nv-create")?.addEventListener("click", async () => {
    ...
    setChamCongNvStatus("Đang tạo tab (copy SU_BEO)…");
    try {
      const res = await api("/api/cham-cong-employees", {
        method: "POST",
