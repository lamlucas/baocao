...
const state = {
  ...
  thuChiPanelReportMode: "monthly",
  thuChiNguonFilter: "",
  thuChiPanelNav: { month: "", day: "" },
...
function thuChiRowNguonKey(r) {
  const t = String(r.ghiChu ?? "").trim();
  return t || "—";
}
...
function thuChiRowsForReport() {
  ...
  if (state.thuChiPanelReportMode !== "nguon") return rows;
  const nguon = state.thuChiNguonFilter || "";
  if (!nguon) return [];
  return rows.filter((r) => thuChiRowNguonKey(r) === nguon);
}
...
function populateThuChiNguonFilter() {
  const sel = $("#thu-chi-nguon-filter");
  if (!sel) return;
  const prev = state.thuChiNguonFilter || "";
  ...
  if (prev && list.includes(prev)) sel.value = prev;
    ...
    sel.value = "";
    state.thuChiNguonFilter = "";
  }
}

function setThuChiPanelReportMode(mode) {
  state.thuChiPanelReportMode = mode === "nguon" ? "nguon" : "monthly";
  state.thuChiPanelNav = { month: "", day: "" };
  document.querySelectorAll(".thu-chi-mode-btn").forEach((btn) => {
    const on = btn.dataset.mode === state.thuChiPanelReportMode;
    btn.classList.toggle("active", on);
    btn.setAttribute("aria-pressed", on ? "true" : "false");
  });
  const wrapFilter = $("#wrap-thu-chi-nguon-filter");
  if (wrapFilter) wrapFilter.hidden = state.thuChiPanelReportMode !== "nguon";
  renderThuChiPanelReport();
}

function setThuChiPanelNav(month, day) {
  state.thuChiPanelNav = { month: month || "", day: day || "" };
  renderThuChiPanelReport();
}

function renderThuChiPanelReport() {
  const { month, day } = state.thuChiPanelNav;
  const wrapMonths = $("#wrap-thu-chi-panel-months");
  const wrapDays = $("#wrap-thu-chi-panel-days");
  const wrapDetail = $("#wrap-thu-chi-panel-detail");
  ...
  const needNguon = state.thuChiPanelReportMode === "nguon" && !state.thuChiNguonFilter;

  const nguonLabel = $("#thu-chi-panel-nguon-label");
  if (nguonLabel) {
    if (state.thuChiPanelReportMode === "nguon" && state.thuChiNguonFilter) {
      nguonLabel.hidden = false;
      nguonLabel.textContent = ` — ${state.thuChiNguonFilter}`;
    } else {
  ...
  const tbMonth = $("#table-thu-chi-panel-month tbody");
  if (!month && tbMonth) {
    tbMonth.innerHTML = "";
    if (needNguon) {
  ...
  const monthLabel = $("#thu-chi-panel-month-label");
  if (monthLabel) monthLabel.textContent = month ? formatMonthForDisplay(month) : "—";

  const tbDays = $("#table-thu-chi-panel-days tbody");
  if (month && !day && tbDays) {
    tbDays.innerHTML = "";
    const days = byDay
  ...
  const dayLabel = $("#thu-chi-panel-day-label");
  ...
  if (day) {
    ...
    const elThu = $("#thu-chi-panel-day-thu");
    const elChi = $("#thu-chi-panel-day-chi");
    if (elThu) elThu.textContent = `Tổng thu: ${fmtMoney(tThu)}`;
    if (elChi) elChi.textContent = `Tổng chi: ${fmtMoney(tChi)}`;
    renderThuChiDetailTable($("#table-thu-chi-panel-detail tbody"), dayRows);
  }
}
...
function bindToolbarAfterLogin() {
  ...
  $("#btn-thu-chi-mode-monthly")?.addEventListener("click", () => setThuChiPanelReportMode("monthly"));
  $("#btn-thu-chi-mode-nguon")?.addEventListener("click", () => setThuChiPanelReportMode("nguon"));
  $("#thu-chi-nguon-filter")?.addEventListener("change", (ev) => {
    state.thuChiNguonFilter = ev.target.value || "";
    setThuChiPanelNav("", "");
  });
  $("#table-thu-chi-panel-month")?.addEventListener("click", (ev) => {
    const tr = ev.target.closest("tr.report-tc-click-row");
    if (!tr?.dataset.month) return;
    setThuChiPanelNav(tr.dataset.month, "");
  });
  $("#table-thu-chi-panel-days")?.addEventListener("click", (ev) => {
    const tr = ev.target.closest("tr.report-tc-click-row");
    if (!tr?.dataset.day) return;
    setThuChiPanelNav(state.thuChiPanelNav.month, tr.dataset.day);
  });
  $("#btn-thu-chi-panel-back-month")?.addEventListener("click", () => setThuChiPanelNav("", ""));
  $("#btn-thu-chi-panel-back-days")?.addEventListener("click", () =>
    setThuChiPanelNav(state.thuChiPanelNav.month, ""),
  );
}
