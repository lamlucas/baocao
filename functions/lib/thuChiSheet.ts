...
export function num(s: string | undefined): number {
  ...
  if (lastDot !== -1 && lastComma !== -1) {
    ...
    /** 1.234.567 — dấu phân cách nghìn lặp (chỉ dấu chấm). */
    if (sep === "." && parts.length > 2 && parts.slice(1).every((p) => p.length === 3)) {
      t = parts.join("");
    } else if (parts.length === 2 && /^\d+$/.test(parts[0]) && /^\d+$/.test(parts[1])) {
      /** 6350,635 hoặc 6350.635 — phần thập phân, không ghép thành 6350635. */
      t = `${parts[0]}.${parts[1]}`;
    } else {
      t = raw.replace(sep, ".");
    }
...
/** Chuỗi `dd/MM/yyyy` cho USER_ENTERED — Sheet VN thường giữ định dạng cột Ngày (không thành số trần). */
export function isoToSheetDateInput(iso: string): string {
...
/** Serial ngày Google Sheet — dùng valueInputOption RAW (giữ định dạng cột). */
export function isoToSheetDateSerial(iso: string): number | string {
  const core = flexibleDateToIso(iso);
  const m = core.match(/^(\d{4})-(\d{2})-(\d{2})$/);
  if (!m) return iso.trim();
  const y = Number(m[1]);
...
/** Số cột đọc BAO_CAO_TK (A–M). */
export const BAO_CAO_TK_COLS = 13;

/** Chuẩn hóa một dòng BAO_CAO_TK: A ngày … M nguồn. */
export function normalizeBaoCaoTkDataRow(cells: unknown): string[] {
  const src = Array.isArray(cells) ? cells : [];
  const row = [...src];
  while (row.length < BAO_CAO_TK_COLS) row.push("");
  const a = row[0];
  let ngay = "";
  if (typeof a === "number" && Number.isFinite(a)) {
    if (a > 35000 && a < 65000 && Math.floor(a) === a) {
      ngay = sheetsSerialToIsoDate(a);
    } else {
      ngay = String(a);
    }
...
export function buildBanDaoAppendedMatrix(
  ...
  for (let i = 1; i < existing.length; i++) {
    const norm = normalizeBanDaoDataRow(existing[i]);
  ...
  for (const r of appendedBodyRows) {
    ...
    const cells = o.slice(0, BAN_DAO_COLS);
    cells[0] = isoToSheetDateInput(String(cells[0] ?? ""));
    body.push(cells.map((c) => String(c ?? "")));
  }
  return padMatrix([h, ...body], BAN_DAO_COLS);
}

export type BanDaoAppendFields = {
  ten: string;
  diaChi: string;
  sdt: string;
  soLuong: string;
...
/** Một dòng BAN_DAO để append (không ghi đè bảng). */
export function buildBanDaoAppendRow(ngay: string, f: BanDaoAppendFields): (string | number)[] {
  return [
...
/** Dòng CÔNG NỢ mới để append (không ghi đè bảng). */
export function buildCongNoAppendRows(
...
/** Một dòng THU_CHI để append (không ghi đè bảng → giữ định dạng cột Ngày). */
export function buildThuChiAppendRow(m: ThuChiSheetModel): (string | number)[] {
  return [
    isoToSheetDateInput(m.ngay),
    m.thu.trim() === "" ? "" : num(m.thu),
    m.chi.trim() === "" ? "" : num(m.chi),
...
/** Các dòng COC để append (không ghi đè bảng → giữ định dạng cột Ngày). */
export function buildCocAppendRows(appended: CocSheetModel[]): (string | number)[][] {
