...
/** Ma trận đầy đủ tab THU_CHI (header + đủ dòng pad): A–D từ model, E luôn là công thức. */
export function buildThuChiPaddedMatrix(
  ...
  const bodyLines = Math.max(THU_CHI_PAD_ROWS - 1, rows.length);
  for (let i = 0; i < bodyLines; i++) {
    ...
    if (model) {
      ...
      out.push(["", "", "", "", thuChiBalanceFormula(sheetRow)]);
    }
  }
  return out;
}

/** Số dư cuối sau các dòng model (dùng cập nhật TONG_QUAN E2; không đọc từ công thức E). */
export function closingBalanceFromModels(
  rows: { ngay: string; thu: string; chi: string; ghiChu: string }[],
  duDau: number,
): number {
  let balance = duDau;
