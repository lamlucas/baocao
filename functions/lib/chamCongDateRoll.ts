...
function todayExistsInRows(rows: unknown[][], today: DateParts): boolean {
...
export async function ensureTodayDateRowsForTab(
  ...
  if (todayExistsInRows(rows, today)) return 0;
  ...
  const valueRows = toAdd.map((ngay) => [ngay, false, "", "", "", ""]);
  await sheetsSpreadsheetBatchUpdate(accessToken, spreadsheetId, [
    {
      updateCells: {
        range: {
          sheetId,
          startRowIndex: insertAfter,
          endRowIndex: insertAfter + toAdd.length,
          startColumnIndex: 0,
