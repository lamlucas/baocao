export interface Env {
  ...
  /**
   * JSON map chat_id → mã đại lý (cột A CONG_NO), ví dụ `{"-1001234567890":"NVT"}`.
   * Dùng khi tin TỔNG TIỀN không kèm MÃ ĐL (mã lấy thêm từ tên nhóm `BLA - NVT` → NVT).
   */
  TELEGRAM_AGENT_CHAT_MAP?: string;
}
