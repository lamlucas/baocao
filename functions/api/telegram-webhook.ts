...
/** Chỉ nối dòng mới ở cuối BAO_CAO_TK; cột E (RATE) ghi số thực (RAW) để không ép thành văn bản. */
function buildBaoCaoTkAppendMatrix(
  existing: unknown[][],
  mcc: string,
  taiKhoan: string[],
  tenPairs: { d: string; e: string }[],
  ...
  const header = existing[0]?.length
    ...
    : ["Ngày", "MCC", "Tài khoản", "Tên", "RATE"];
  while (header.length < 5) header.push("");
  const h = header.slice(0, 5).map(String);
  const body: (string | number)[][] = [];
  for (let r = 1; r < existing.length; r++) {
    const cells = existing[r] as unknown[];
    if (!Array.isArray(cells)) continue;
    const str = stringifySheetRow(cells).slice(0, 5);
    while (str.length < 5) str.push("");
    if (!str.join("").trim()) continue;
    const eCell = cells[4];
    const eVal: string | number | "" =
      typeof eCell === "number" && Number.isFinite(eCell)
        ? eCell
        : rateNumericOrEmpty(str[4] ?? "");
    body.push([str[0], str[1], str[2], str[3], eVal]);
  }
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  if (sid === baoCaoChatId(env)) {
    ...
    try {
      ...
      const matrix = buildBaoCaoTkAppendMatrix(
        existing,
        parsed.mcc,
        parsed.taiKhoan,
        parsed.tenPairs,
  ...
  try {
    ...
    /** Chỉ cập nhật A2:C2 (Dư đầu / Tổng cọc / Nhận cọc). Không ghi D2:E2 — D2 là công thức Biến động trên Sheet. */
