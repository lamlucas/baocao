...
import {
  applyThuChiNewRowsFillEmptySlots,
...
/** Tab COC: A Ngày, B Thu, C Chi, D Ghi chú — THU → cột B, CHI → cột C.
 * Một dòng: CỌC: THU - 1000 - RF BOX WOLF / CỌC: CHI - 1000 - ...
 * Nhiều dòng: dòng 1 bắt đầu CỌC:, có thể chỉ "CỌC:" hoặc "CỌC: THU - ..."; các dòng sau: THU|CHI - số - ghi chú */
function parseCocMessage(text: string): { lines: { kind: "THU" | "CHI"; amountStr: string; note: string }[] } | null {
  const lines = text.split(/\r?\n/).map((l) => l.trim()).filter(Boolean);
  if (lines.length === 0) return null;

  const pushThuChiLine = (
    line: string,
    out: { kind: "THU" | "CHI"; amountStr: string; note: string }[],
  ) => {
  ...
  // Một dòng duy nhất: CỌC: THU - 1000 - ...
  if (lines.length === 1) {
    ...
    return { lines: [{ kind, amountStr, note }] };
  }
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    if (cocBlock) {
      ...
      const cocModels = applyThuChiNewRowsFillEmptySlots(sheetRowsToThuChiModels(cocData), newCocRows);
      ...
      await telegramSendMessage(
        botToken,
        chatId,
        `Đã ghi CỌC (${cocBlock.lines.length} dòng) — ưu tiên ô trống, không ghi đè dòng cũ.`,
      );
    ...
    if (thuChiOne) {
      ...
      const models = applyThuChiNewRowsFillEmptySlots(sheetRowsToThuChiModels(thuChiData), [
      ...
      await telegramSendMessage(
        botToken,
        chatId,
        `Đã ghi ${label} ${amount} — ${thuChiOne.note} (ngày ${ngay}); ưu tiên dòng trống, không ghi đè dòng cũ.`,
      );
