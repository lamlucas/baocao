...
/** File VK8: THU_CHI + COC + CONG_NO + BAN_DAO. `null` = layout cũ (TC/COC trên MAIN, CN trên DEBT_SALES). */
...
function buildBaoCaoOverwriteMatrix(
  existing: unknown[][],
  mcc: string,
  taiKhoan: string[],
  tenPairs: { d: string; e: string }[],
  ...
  for (let i = 0; i < n; i++) {
    ...
    body.push([ngay, mcc, taiKhoan[i] ?? "", tenPairs[i]?.d ?? "", eNum === 0 && eRaw.trim() === "" ? "" : eNum]);
  }
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  if (sid === banDaoChatId(env)) {
    ...
    try {
      ...
      const idBd = spreadsheetIdLedger(env) ?? env.SPREADSHEET_ID_DEBT_SALES;
  ...
  if (sid === baoCaoChatId(env)) {
    ...
    try {
      const token = await getSheetsAccessToken(env.GOOGLE_SERVICE_ACCOUNT_JSON);
      const batch = await sheetsBatchGet(token, env.SPREADSHEET_ID_DEBT_SALES, [`'${SHEET_BAO_CAO_TK}'!A1:E500`]);
      const existing = batch[SHEET_BAO_CAO_TK] ?? [];
      const ngay = formatNgayFromTelegram(unix);
      const matrix = buildBaoCaoOverwriteMatrix(
        existing,
        parsed.mcc,
        parsed.taiKhoan,
        parsed.tenPairs,
      ...
      await telegramSendMessage(botToken, chatId, `Đã ghi đè BAO_CAO_TK (${matrix.length - 1} dòng dữ liệu).`);
  ...
  try {
    ...
    const idVk = spreadsheetIdLedger(env);
    const idDebt = env.SPREADSHEET_ID_DEBT_SALES;
    const idTcCoc = idVk ?? idMain;
    const idCn = idVk ?? idDebt;
    ...
    let cnRaw: unknown[][];
    if (idVk) {
      const batchLedger = await sheetsBatchGetMergeSafe(token, idVk, [
        `'${SHEET_TC}'!A1:D2000`,
        `'${SHEET_COC}'!A1:D2000`,
        `'${SHEET_CN}'!A1:B2000`,
      ]);
      tc = batchLedger[SHEET_TC] ?? [];
      cocRaw = batchLedger[SHEET_COC] ?? [];
      cnRaw = batchLedger[SHEET_CN] ?? [];
    } else {
      const batchMain = await sheetsBatchGetMergeSafe(token, idMain, [
        `'${SHEET_TC}'!A1:D2000`,
        `'${SHEET_COC}'!A1:D2000`,
      ]);
      const batchDebt = await sheetsBatchGetMergeSafe(token, idDebt, [`'${SHEET_CN}'!A1:B2000`]);
      tc = batchMain[SHEET_TC] ?? [];
      cocRaw = batchMain[SHEET_COC] ?? [];
      cnRaw = batchDebt[SHEET_CN] ?? [];
    }
    ...
    if (congNoBlock) {
      ...
      await sheetsBatchUpdate(token, idCn, [
    ...
    if (cocBlock) {
      ...
      await sheetsBatchUpdate(token, idTcCoc, [
