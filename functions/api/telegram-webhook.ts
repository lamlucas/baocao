...
import {
  clearCongNoDebtIfThuMatchesWithinTwo,
  parseAgentChatMapJson,
  parseCongNoUpsertWithContext,
  upsertCongNoDebt,
  upsertCongNoDebtPairs,
} from "../lib/congNoTracking";
import { messageBodyText } from "../lib/telegramMessageText";
...
function agentChatMap(env: Env): Record<string, string> {
  return parseAgentChatMapJson((env as { TELEGRAM_AGENT_CHAT_MAP?: string }).TELEGRAM_AGENT_CHAT_MAP);
}

async function telegramGetChatPinnedBody(botToken: string, chatId: number): Promise<string> {
  ...
  const data = (await res.json()) as {
    ok?: boolean;
    result?: { pinned_message?: TelegramMessage };
  };
  const pinned = data.result?.pinned_message;
  return messageBodyText(pinned);
}

/** Tin có dòng TỔNG TIỀN CẦN THANH TOÁN → upsert CONG_NO (mọi nhóm đại lý, kể cả tin bot Báo cáo + caption ảnh). */
async function tryUpsertCongNoFromTelegramMessage(
  env: Env,
  botToken: string,
  msg: TelegramMessage & { reply_to_message?: TelegramMessage },
  sid: string,
): Promise<{ agent: string; debt: number } | null> {
  const body = messageBodyText(msg);
  if (!body && !msg.reply_to_message) return null;

  const texts: string[] = [];
  if (body) texts.push(body);
  const replyBody = messageBodyText(msg.reply_to_message);
  if (replyBody) texts.push(replyBody);

  const pinned = await telegramGetChatPinnedBody(botToken, Number(sid));
  if (pinned) texts.push(pinned);

  const parsed = parseCongNoUpsertWithContext(texts, {
    chatId: sid,
    chatTitle: msg.chat?.title,
    agentByChatId: agentChatMap(env),
  });
  if (!parsed) return null;

  const token = await getSheetsAccessToken(env.GOOGLE_SERVICE_ACCOUNT_JSON);
  await upsertCongNoDebt(token, env.SPREADSHEET_ID_DEBT_SALES, SHEET_CN, parsed.agent, parsed.debt);
  return parsed;
}
...
async function writeThuChiEntryToSheet(env: Env, unix: number, thuChiOne: ThuChiParsed): Promise<void> {
  ...
  const cocRaw = batchCoc[SHEET_COC] ?? [];
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  const thuChiParsed = text ? parseThuChiMessage(text) : null;
  const replyTo = msg?.reply_to_message;

  if (chatId == null || unix == null) {
  ...
  /**
   * Thu:/Chi: trong nhóm đại lý (không phải hub Thu chi) — trả lời ảnh bằng lệnh:
   * chuyển ảnh gốc + lệnh sang « Black Corp - Thu Chi », append dòng THU_CHI (B/C/D, ngày VN).
   */
  if (thuChiParsed && sid !== thuChiDest) {
    ...
    try {
      ...
      const forwardLine = text || `${thuChiParsed.kind}: ${thuChiParsed.amountStr} - ${thuChiParsed.note}`;
      await telegramSendMessage(botToken, destId, forwardLine);
      await writeThuChiEntryToSheet(env, unix, thuChiParsed);
      await telegramSendMessage(botToken, chatId, "Đã nhận thông tin");
      if (thuChiParsed.kind === "THU") {
        try {
          const tokenCn = await getSheetsAccessToken(env.GOOGLE_SERVICE_ACCOUNT_JSON);
          await clearCongNoDebtIfThuMatchesWithinTwo(
            tokenCn,
            env.SPREADSHEET_ID_DEBT_SALES,
            SHEET_CN,
            num(thuChiParsed.amountStr),
            thuChiParsed.note,
          );
        } catch (e) {
          console.error("cong_no_clear_after_thu", e instanceof Error ? e.message : String(e));
        }
      }
  ...
  /** Mọi nhóm (trừ Bán dao): bot Báo cáo gửi `TỔNG TIỀN CẦN THANH TOÁN` (+ ảnh/caption) → cập nhật CONG_NO. */
  if (sid !== banDaoChatId(env)) {
    try {
      const congNoAuto = await tryUpsertCongNoFromTelegramMessage(env, botToken, msg!, sid);
      if (congNoAuto) {
        const isBaoCaoHub = sid === baoCaoChatId(env);
        const hasBaoCaoTkBlock = Boolean(parseBaoCaoMessage(text));
        if (!isBaoCaoHub || !hasBaoCaoTkBlock) {
          await telegramSendMessage(
            botToken,
            chatId,
            `Đã cập nhật CONG_NO — ${congNoAuto.agent}: nợ = ${congNoAuto.debt}`,
          );
          return Response.json({ ok: true, kind: "cong_no_auto", agent: congNoAuto.agent });
        }
      }
    } catch (e) {
      const msgErr = e instanceof Error ? e.message : String(e);
      console.error("cong_no_auto", msgErr);
      await telegramSendMessage(botToken, chatId, `Lỗi cập nhật CONG_NO: ${msgErr}`);
      return Response.json({ ok: false, error: msgErr }, { status: 500 });
    }
  }
  ...
  if (sid === baoCaoChatId(env)) {
    ...
    const congNoFromTong = parseCongNoUpsertWithContext([text], {
      chatId: sid,
      chatTitle: msg?.chat?.title,
      agentByChatId: agentChatMap(env),
    });

    if (!parsed && !congNoFromTong) {
      await telegramSendMessage(
        botToken,
        chatId,
        "Gửi theo khối:\nMCC: …\nTÀI KHOẢN:\n(id dòng 1)\n(id dòng 2)\nTÊN:\nTP - 57\nAKR - 57\n\nHoặc mẫu theo nhóm (mã đại lý = cột A CONG_NO, không dùng MCC):\nMÃ ĐL: AT\n…\nCÔNG NỢ: 1234\nvà/hoặc TỔNG TIỀN CẦN THANH TOÁN: … + … = …",
      );
      return Response.json({ ok: true, ignored: true });
    }
    try {
      ...
      if (parsed) {
        const addedRows = Math.max(parsed.taiKhoan.length, parsed.tenPairs.length, 1);
        parts.push(`Đã thêm ${addedRows} dòng vào BAO_CAO_TK (append — không ghi đè định dạng % / ngày).`);
      }
      if (congNoFromTong) {
        parts.push(
          `Đã cập nhật CONG_NO — ${congNoFromTong.agent}: nợ = ${congNoFromTong.debt} (ghi đè B nếu đã có tên A; nếu chưa có thì nối dòng mới).`,
        );
      }
      await telegramSendMessage(botToken, chatId, parts.join("\n"));
      return Response.json({
        ok: true,
        kind: parsed && congNoFromTong ? "bao_cao_cong_no" : parsed ? "bao_cao" : "cong_no_baocao",
      });
    } catch (e) {
      const msgErr = e instanceof Error ? e.message : String(e);
      console.error("telegram bao_cao", msgErr);
      await telegramSendMessage(botToken, chatId, `Lỗi ghi Sheet (Báo cáo / CONG_NO): ${msgErr}`);
      return Response.json({ ok: false, error: msgErr }, { status: 500 });
    }
  }
  ...
  try {
    ...
    if (congNoBlock) {
      ...
      await upsertCongNoDebtPairs(token, env.SPREADSHEET_ID_DEBT_SALES, SHEET_CN, congNoBlock.pairs);
      await sheetsBatchUpdate(token, idMain, [{ range: `'${SHEET_TQ}'!A2:C2`, values: tqRow2 }]);
      await telegramSendMessage(
        botToken,
        chatId,
        `Đã cập nhật ${congNoBlock.pairs.length} dòng CONG_NO (trùng A thì sửa B; chưa có thì nối dòng mới).`,
      );
    ...
    if (thuChiOne) {
      ...
      await telegramSendMessage(
        botToken,
        chatId,
        `Đã ghi ${label} ${amount} — ${thuChiOne.note} (ngày ${ngay}); dòng mới tab THU_CHI (cột ${thuChiOne.kind === "THU" ? "B" : "C"}).`,
      );
      if (thuChiOne.kind === "THU") {
        try {
          ...
          console.error("cong_no_clear_thu_chi_hub", e instanceof Error ? e.message : String(e));
        }
      }
