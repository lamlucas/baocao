...
function hhLoaiTruRange(): string {
  return `${quoteSheetTitle(HH_LOAI_TRU_TAB)}!A1:D${HH_LOAI_TRU_MAX_ROW}`;
}
...
async function readHhLoaiTruFromSheet(
  ...
  return parseHhLoaiTruSheetRows(
    rawBatch[HH_LOAI_TRU_TAB] ?? [],
    fmtBatch[HH_LOAI_TRU_TAB] ?? [],
  );
}
