...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    const batch = await sheetsBatchGetMergeSafe(token, context.env.SPREADSHEET_ID_MAIN, [
...
export const onRequestPut: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    await sheetsBatchUpdate(
      token,
      context.env.SPREADSHEET_ID_MAIN,
      [{ range: `'${HH_LOAI_TRU_TAB}'!A1:C${Math.max(matrix.length, 1)}`, values: matrix }],
      "USER_ENTERED",
    );
    return Response.json({ ok: true, exclusions: rules });
  } catch (e) {
    ...
    return Response.json(
      {
        error: `${msg} — tạo tab «${HH_LOAI_TRU_TAB}» trên file MAIN (A: Ngày, B: Tên đại lý, C: Khoản thu).`,
      },
      { status: 500 },
    );
  }
