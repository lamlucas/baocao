...
function buildThuChiRowsWithBalance(
  ...
  const out: string[][] = [["Ngày", "Thu", "Chi", "Ghi chú", "Balance fluctuations"]];
  let balance = duDau;
  for (const r of rows) {
    const ngay = r.ngay ?? "";
    const thu = r.thu ?? "";
    ...
    if (hasValue) {
      ...
      out.push([ngay, thu, chi, ghiChu, String(balance)]);
    } else {
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  const d2 = `=SUM(IMPORTRANGE("${env.SPREADSHEET_ID_DEBT_SALES}";"CONG_NO!B2:B"))`;
  const e2 = '=LOOKUP(2;1/(THU_CHI!E2:E<>"");THU_CHI!E2:E)';

  const tqName = SHEETS.tong_quan;
  const thuChiName = SHEETS.thu_chi;
  const cocName = SHEETS.coc;

  const thuChiRows: string[][] = buildThuChiRowsWithBalance(thuChi, num(a2));
