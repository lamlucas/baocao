...
async function loadAttendanceSheets(
  ...
  const ranges = titles.map((t) => `${quoteSheetTitle(t)}!A1:E400`);
...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  try {
    ...
    const hhLoaiTru = parseHhLoaiTruSheetRows(batchMain[HH_LOAI_TRU_TAB] ?? []);

    let reportLuongNv = buildLuongNvReport([], thuChiModels, todayVn, hhLoaiTru);
    try {
      const attendanceSheets = await loadAttendanceSheets(token, spreadsheetIdChamCong(env));
      reportLuongNv = buildLuongNvReport(attendanceSheets, thuChiModels, todayVn, hhLoaiTru);
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
