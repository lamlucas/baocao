...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  const thuChiDataFromSheet =
    skipThuChi && tcFromSheet.length > 1 ? parseRows(tcFromSheet.slice(1), 5) : [];
  ...
  const thuChiRows = buildThuChiRowsWithBalance(thuChiModels, num(a2));
  ...
  const sumCongNoB = congNoData.reduce((s, r) => s + num(r[1]), 0);
  const latestBalance = latestBalanceFromBuiltMatrix(thuChiRows, num(a2));
