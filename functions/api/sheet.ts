...
import { buildChiTieuReport, parseBaoCaoTkSheetRows } from "../lib/baoCaoTkReport";
import {
  flexibleDateToIso,
...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  try {
    ...
    const batchDebtCn = await sheetsBatchGetMergeSafe(token, idDebt, [
      `'${SHEETS.cong_no}'!A1:B2000`,
      `'${SHEETS.bao_cao_tk}'!A1:L5000`,
    ]);
    ...
    const baoCaoTkRaw = batchDebtCn[SHEETS.bao_cao_tk] ?? [];
    ...
    const baoCaoTkBody = baoCaoTkRaw.length > 1 ? baoCaoTkRaw.slice(1) : [];
    const baoCaoTkEntries = parseBaoCaoTkSheetRows(baoCaoTkBody);
