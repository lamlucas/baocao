...
import { getSheetsAccessToken, sheetsBatchGetMergeSafe, sheetsBatchGetValueRanges } from "../lib/google";
import {
  baoCaoTkColValues,
  buildChiTieuReport,
...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  try {
    ...
    const baoCaoTkHeader = baoCaoTkMerged[0] ?? [];
    const baoCaoTkBody = baoCaoTkMerged.length > 1 ? baoCaoTkMerged.slice(1) : [];
