...
async function loadAttendanceSheets(
  ...
  const ranges = titles.map((t) => `${quoteSheetTitle(t)}!A1:B400`);
...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  try {
    ...
    let reportLuongNv = buildLuongNvReport([], thuChiModels, todayVn);
    try {
      const attendanceSheets = await loadAttendanceSheets(token, spreadsheetIdChamCong(env));
      reportLuongNv = buildLuongNvReport(attendanceSheets, thuChiModels, todayVn);
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
